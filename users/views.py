    ...
    if request.method == 'POST':
        ...
        if obj.user in my_profile.following.all():
            ...
            notify = Notification.objects.filter(
                sender=request.user, notification_type=2
            )
            notify.delete()
        else:
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Your account has been created! "
                         f"Please Update Profile and add a profile picture."
            )
            # login created user with backend
            login(
                request, user,
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated and user != account:
            ...
            if friends.filter(pk=user.id):
                ...
                if get_friend_request_or_false(
                ...
                elif get_friend_request_or_false(
